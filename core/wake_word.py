"""
Local wake-word detection for LangVis ("Hey Talvis").
...
CPU. The pretrained wake phrase used here is "Hey Talvis".
"""
...
# The wake phrase is NOT the assistant's name and cannot be renamed by editing
# a string. openwakeword ships exactly six pretrained detectors — alexa,
# hey_mycroft, hey_jarvis, hey_rhasspy, timer, weather — and "hey_jarvis" is
# the closest to an assistant call sign. Saying anything else wakes nothing.
#
# WAKE_PHRASE is what the user must actually say, and every message that tells
# them so is built from it, so a future custom-trained model ("Hey Talvis")
# becomes a two-line change here instead of a hunt through the UI strings.
WAKE_MODEL = "hey_jarvis"
WAKE_PHRASE = "Hey Talvis"
        ...
        while self._running:
            try:
                ...
                if isinstance(scores, dict):
                    # match the langvis model regardless of exact key suffix
                    for k, v in scores.items():
                        if "langvis" in k.lower():
